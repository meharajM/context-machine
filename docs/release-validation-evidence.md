...
The current working tree was revalidated locally on 2026-07-08 after the
release-automation alignment changes. These commands passed:

- `npm run verify`
- `npm publish --dry-run --access public`

This keeps the local package/build/test baseline green, but it does not change
the remaining external host, Google Drive, PMF, or mobile release gates.
...
| --- | --- | --- |
| Claude-family host | Blocked | Claude Code is installed at `/usr/local/bin/claude`, but `claude --print` still returned `Credit balance is too low` on 2026-07-08. A working Claude account balance is required to complete the host workflow. |
| Cursor host | Blocked | The `cursor` CLI is not installed on this machine (`cursor: command not found` on 2026-07-08). Install Cursor/Agent on this machine or validate on another machine where Cursor is available, then log in or provide `CURSOR_API_KEY`. |
...
For Cursor after the CLI is installed on the validation machine:
